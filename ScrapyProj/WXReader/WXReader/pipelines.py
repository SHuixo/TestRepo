...
class MySQLPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if len(self.data_list) == 100:
            ...
            self.data_list.append((asyncitem["uid"], asyncitem["classify"], asyncitem["type"], asyncitem["name"], asyncitem["author"], asyncitem["publish"],
                                   asyncitem["ptime"], asyncitem['price'], asyncitem['isbn'], asyncitem['country'], asyncitem['label'], asyncitem['score'],
                                   asyncitem['commNum'], asyncitem['readNum'], asyncitem['collNum'], asyncitem["app"]))
        return asyncitem

    # 写入数据库中
    def _conditional_insert(self, tx, item):
        sql = "insert ignore into wxreader(uid, classify,type, name, author, publish, " \
              "ptime, price, isbn, country, label, score, commNum, readNum, collNum, app) " \
